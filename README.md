...
Локально работающий инструмент для восстановления свободного обмена информацией и открытого доступа к YouTube, Discord, и т. д.  
В настоящее время находится в разработке.

_Проект создан исключительно для восстановления стабильного доступа к привычным образовательным, коммуникационным, игровым и медиа-платформам. Инструмент не предназначен для обхода ограничений на опасные и вредоносные ресурсы. Автор не несет ответственности за нецелевое использование утилиты._
...
- Windows 11 24H2 ARM64 с включенным testsigning `bcdedit /set {current} testsigning on`
...
#### Поддержка Linux / Mono (в разработке):
- *ubuntu 20.04+ amd64 и aarch64
- Debian 11+ amd64 и aarch64
...
Перебирайте / переключайте с задержкой профили до тех пор, пока не заработают необходимые сайты.  
Если провайдер перехватывает или подменяет DNS запросы - используйте шифрованный DNS ([DoT](https://ru.wikipedia.org/wiki/DNS_поверх_TLS)/[DoH](https://ru.wikipedia.org/wiki/DNS_поверх_HTTPS)).  
...
- Ярлык с именем `liberty-pre SWITCH - Extended Ports` (отдельно `TCP` и `UDP`) предназначен для переключения расширенного режима фильтрации портов (1024-65535), в осномном для игр.  
- Ярлык с именем `liberty-pre SWITCH - hidden mode` предназначен для переключения невидимого / скрытого режима работы.  
- Ярлык с именем `liberty-pre SWITCH - hostlist-auto` предназначен для переключения режима autohostlist (автоматическая разблокировка сайтов). Работает медленно, но в редких случаях может помочь.  
...
### Вклад
Этот проект носит любительский характер и в первую очередь ориентирован на простоту и кросс-платформенность.

Если вы хотите внести изменения или улучшения - это всегда приветствуется!  
Пожалуйста, по возможности сохраняйте совместимость с .NET Framework 4.5 / Mono (C# 7.3-8), а также старайтесь избегать излишнего усложнения кода.  
В случае сомнений лучше обсудить изменения заранее.
